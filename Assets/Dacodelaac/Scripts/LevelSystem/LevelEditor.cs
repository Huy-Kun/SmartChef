...
namespace Dacodelaac.LevelSystem
{
    ...
    {
        string AssetPath => "Assets/Level-Prefabs";
        string FolderPath => Path.Combine(Application.dataPath, "Level-Prefabs");
        ...
        void OnGUI()
        {
            ...
            var level = stageHandle.FindComponentOfType<Level>();
            if (level && PrefabStageUtility.GetCurrentPrefabStage() != null)
            {
                var levelAsset =
                    AssetDatabase.LoadAssetAtPath<Level>(PrefabStageUtility.GetCurrentPrefabStage().assetPath);
                if (levelAsset)
                {
                    GUI.enabled = false;
                    EditorGUILayout.ObjectField("Level", levelAsset, typeof(Level), true);
                    GUI.enabled = true;
        ...
        void DrawGameSpecific(Level level)
        {
        }
        ...
        void TransformHelper(Level level)
        {
            var ec = Event.current;
        ...
        Vector3 RaycastPoint(Level level, Vector2 screenPoint, float dist = 10)
        {
        ...
        bool RayCast(Level level, Ray r, out Vector3 hitPoint)
        {
