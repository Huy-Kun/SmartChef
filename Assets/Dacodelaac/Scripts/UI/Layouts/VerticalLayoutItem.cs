...
{
    public class VerticalLayoutItem : BaseMono
    {
        ...
        public void Scaling(Vector3 scale, bool animated, System.Action onCompleted)
        {
            ...
            if (animated)
            {
                rt.DOScale(scale, 0.75f).SetTarget(TweenIdScale).OnComplete(() => { onCompleted?.Invoke(); }).Play();
            }
            else
            {
        ...
        public void MoveY(float y, bool animated)
        {
            ...
            if (animated)
            {
                rt.DOAnchorPosY(targetY, 0.75f).SetTarget(TweenIdY).Play();
            }
            else
            {
