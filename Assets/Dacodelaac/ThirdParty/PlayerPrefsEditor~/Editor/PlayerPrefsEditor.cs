...
public class PlayerPrefsEditor : EditorWindow
{
	...
	private PlayerPrefPair[] RetrieveSavedPrefs(string companyName, string productName)
    {
		if(Application.platform == RuntimePlatform.OSXEditor)
		{
			...
			string playerPrefsPath = Path.Combine(Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.Personal), "Library/Preferences"), plistFilename);
	...
	private void DrawMainList()
    {
        ...
        float totalBytes = 0;
        foreach(var pref in activePlayerPrefs)
        {
            totalBytes += pref.Key.Length * 2f;
            if(pref.Value is string)
            {
                totalBytes += ((string)pref.Value).Length * 2f;
            }
            else if(pref.Value is int)
            {
                totalBytes += 8;
            }
            else if(pref.Value is float)
            {
                totalBytes += 4;
            }
        }

        GUILayout.Label("Size(KB): " + (totalBytes / 1024).ToString("N2"));
    }
